...
void oldSkills(){
   ...
   pros::delay(150);
   stopIntake();
   chassis.moveToPoint(20, 19, 1200, {.forwards = false, .maxSpeed = 70, .minSpeed = 50, .earlyExitRange = 0.3});
   chassis.turnToHeading(-88, 900, {.maxSpeed = 70});
   chassis.waitUntilDone();
   chassis.moveToPose(60, 19, -88,800, {.forwards = false, .lead = 0.1, .maxSpeed = 90, .minSpeed = 70, .earlyExitRange = 2});
   chassis.moveToPose(80, 18.5, -88, 700, {.forwards = false, .lead = 0.1, .maxSpeed = 70, .minSpeed = 50, .earlyExitRange = 2});
   chassis.turnToHeading(1, 600, {.maxSpeed = 70});
   chassis.waitUntilDone();
   while(fabs(frontSens.get())>= 562){
    chassis.tank(90, 90);
   }
   chassis.tank(0,0);
   ...
   chassis.tank(-80, -80);
   pros::delay(150);
   chassis.moveToPoint(80, 31, 800, {.forwards = false, .maxSpeed = 70});
   chassis.turnToHeading(88, 600, {.maxSpeed = 70});
   chassis.moveToPoint(70, 31, 900, {.forwards = false, .maxSpeed = 70});
   chassis.waitUntilDone();
   chassis.tank(-30,-30);
   scoreLongGoal();
   pros::delay(2000);
   ...
   chassis.waitUntilDone();
    chassis.setPose(20, -72, -90);
   scoreLongGoal();
   matchloader.set_value(false);
   pros::delay(2000);
   double prev_y = -72 + leftSens.get()/25.4;
   chassis.setPose(-72 + frontSens.get()/25.4, -72 + leftSens.get()/25.4, chassis.getPose().theta);
   pros::delay(10);
   chassis.turnToHeading(0, 2000, {.maxSpeed = 127});
   chassis.waitUntilDone();
   chassis.setPose(-72 + leftSens.get()/25.4, prev_y, chassis.getPose().theta);
   storeIntake();
   chassis.moveToPoint(-27, -52, 3000, {.forwards = true, .maxSpeed = 40});
   chassis.waitUntilDone();
   chassis.turnToHeading(0, 600, {.maxSpeed = 70});
   chassis.waitUntilDone();
   matchloader.set_value(false);
   chassis.moveToPoint(-34, 10, 4000, {.forwards = true, .maxSpeed = 40});
   chassis.waitUntilDone();
   pros::delay(100);
   chassis.tank(0,0);
   matchloader.set_value(true);
...
//    manualIntake(127, -50);
   ...
   chassis.tank(0,0);
   matchloader.set_value(false);
   chassis.moveToPoint(-61, 6, 1200, {.forwards = true, .maxSpeed = 120, .minSpeed = 50, .earlyExitRange = 0.2});
...
void diamondSkills(){
   ...
   pros::delay(1900);
    ...
    chassis.swingToHeading(-90, lemlib::DriveSide::RIGHT, 900, {.maxSpeed = 90});
    chassis.waitUntilDone();
   ...
   pros::delay(10);
   chassis.moveToPoint(-35, 50, 1200, {.forwards = false, .maxSpeed = 90, .minSpeed = 30, .earlyExitRange = 1});
   matchloader.set_value(false);
   //chassis.moveToPoint(-2, 50, 1200, {.forwards = false, .maxSpeed = 90, .minSpeed = 30, .earlyExitRange = 1});
   chassis.moveToPoint(35, 50, 1600, {.forwards = false, .maxSpeed = 90});

   chassis.swingToHeading(0, lemlib::DriveSide::LEFT, 1000, {.maxSpeed = 100});
   chassis.waitUntilDone();
   while(fabs(frontSens.get())<= 248){
    chassis.tank(-60, -60);
   }
   chassis.turnToHeading(88, 900, {.maxSpeed = 70});
   chassis.waitUntilDone();
   ...
   //matchloader.set_value(true);
   matchloader.set_value(false);
   moveDistanceWFrontDist(28, 1500, 80);
   chassis.waitUntilDone();
...
//    chassis.tank(0,0);
   ...
   chassis.moveToPoint(40, -44.5, 900, {.forwards = true, .maxSpeed = 100});
   ...
   chassis.setPose(26,-49,chassis.getPose().theta);
   pros::delay(10);
   chassis.moveToPoint(52, -49.5, 1200, {.forwards = true, .maxSpeed = 70, .minSpeed = 50, .earlyExitRange = 0.3});
   matchloader.set_value(true);
   chassis.turnToHeading(90, 600, {.maxSpeed = 70});
   chassis.waitUntilDone();
   storeIntake();
   ...
   chassis.tank(80,80);
   pros::delay(500);
   chassis.moveToPoint(51, -49.5, 1200, {.forwards = false, .maxSpeed = 90, .minSpeed = 50, .earlyExitRange = 0.3});
   chassis.waitUntilDone();
   matchloader.set_value(false);
   chassis.turnToHeading(133, 800, {.maxSpeed = 70});
   ...
   */
   ...
   chassis.turnToHeading(180, 1000, {.maxSpeed = 70});
   chassis.waitUntilDone();
   while(fabs(frontSens.get())>= 580){
    chassis.tank(70, 70);
   }
   chassis.tank(0,0);
   chassis.turnToHeading(-93, 700, {.maxSpeed = 90});
   chassis.waitUntilDone();
   chassis.tank(-90,-90);
   pros::delay(800);
   chassis.tank(-10,-10);
   ...
   chassis.tank(127, 127);
   pros::delay(200);
   chassis.moveToPoint(-63,-23, 1200, {.forwards = true, .maxSpeed = 100});
...
void diamondSkills2(){
    ...
    middleGoal.set_value(false);
    storeIntake();
    chassis.setPose(-52,3, 90);
    pros::delay(10);
    chassis.moveToPoint(-31,13, 2000, {.maxSpeed = 70});
    chassis.waitUntilDone();
    chassis.turnToHeading(-50, 600, {.maxSpeed = 70});
    pros::delay(100);
    stopIntake();
    chassis.moveToPose(-18,4,-45, 900, {.forwards = false, .horizontalDrift = 8, .lead = 0.3, .maxSpeed = 90});
    chassis.waitUntilDone();
    chassis.setPose(-18, 4, chassis.getPose().theta);
    middleGoal.set_value(true);
    // --- first middle goal score ---
    scoreMiddleGoal();
    pros::delay(1500);
    stopIntake();
    middleGoal.set_value(false);
    scoreLongGoal();
    chassis.moveToPoint(-54,40, 1200, {.maxSpeed = 90});
    ...
    chassis.setPose((-67 + (frontSens.get()/25.4) + 5.5), (67 - (rightSens.get()/25.4) - 7),chassis.getPose().theta);
   pros::delay(10);
   chassis.moveToPoint(-35, 31, 1200, {.forwards = false, .maxSpeed = 90, .minSpeed = 30, .earlyExitRange = 1});
   matchloader.set_value(false);
   chassis.turnToHeading(-90, 700, {.maxSpeed = 100});
   //chassis.moveToPoint(-2, 50, 1200, {.forwards = false, .maxSpeed = 90, .minSpeed = 30, .earlyExitRange = 1});
   chassis.moveToPoint(38, 31, 1600, {.forwards = false, .maxSpeed = 90});
   ...
   chassis.moveToPoint(80, 31, 800, {.forwards = false, .maxSpeed = 70});
   chassis.turnToHeading(88, 600, {.maxSpeed = 70});
   chassis.moveToPoint(71, 31, 200, {.forwards = false, .maxSpeed = 70});
   chassis.waitUntilDone();
   chassis.tank(-90,-90);
   // --- second long goal score ---
   skillsLongGoalScore();
   pros::delay(2000);
   ...
   // --- second middle goal score ---
   chassis.turnToHeading(180, 600, {.maxSpeed = 100});
   chassis.waitUntilDone();
   chassis.setPose((67 - (leftSens.get()/25.4) - 5.5), (-67 + (frontSens.get()/25.4) + 7),chassis.getPose().theta);
   pros::delay(10);
   /*
   chassis.swingToHeading(-90, lemlib::DriveSide::RIGHT, 1500, {.maxSpeed = 127});
   chassis.waitUntilDone();
   chassis.tank(-80, -80);
   pros::delay(400);
   chassis.tank(50, 50);
   pros::delay(300);
   chassis.tank(0,0);
   chassis.setPose(65, (-67 + (leftSens.get()/25.4) - 5.5), chassis.getPose().theta);
   pros::delay(10);
   */
   chassis.swingToHeading(-100, lemlib::DriveSide::RIGHT, 2400, {.maxSpeed = 40});
   chassis.moveToPoint(20,-18, 3000, {.forwards = true, .maxSpeed = 60, .minSpeed = 50, .earlyExitRange = 0.3});
   chassis.waitUntilDone();
   ...
   pros::delay(100);
   chassis.tank(-20,-20);
   chassis.setPose(12,-16,chassis.getPose().theta);
   pros::delay(10);
   chassis.moveToPoint(49, -43.7, 1200, {.forwards = true, .maxSpeed = 80});
   ...
   chassis.tank(80,80);
    pros::delay(1600);
    chassis.turnToHeading(90, 400, {.maxSpeed = 100});
    moveDistance(-3, 600, 127);
    chassis.tank(80,80);
    pros::delay(1000);
    chassis.tank(0,0);
   stopIntake();
   chassis.setPose((67 - (frontSens.get()/25.4) - 5.5), (-67 + (rightSens.get()/25.4) + 7),90);
   pros::delay(10);
   chassis.moveToPoint(32, -32, 1200, {.forwards = false, .maxSpeed = 100, .minSpeed = 50, .earlyExitRange = 1});
   matchloader.set_value(false);
   chassis.turnToHeading(90, 800, {.maxSpeed = 100});
   chassis.moveToPoint(-36, -35, 1400, {.forwards = false,  .maxSpeed = 100});
   chassis.turnToHeading(178, 900, {.maxSpeed = 100});
   chassis.waitUntilDone();
   ...
   /*
    ...
    chassis.tank(0,0);
    stopIntake();
    chassis.moveToPoint(40, -50, 1200, {.forwards = false, .maxSpeed = 80});
    chassis.turnToHeading(180,800, {.maxSpeed = 90});
    chassis.moveToPoint(40, -60, 1200, {.forwards = false, .maxSpeed = 80});
    chassis.turnToHeading(270,800, {.maxSpeed = 90});
    chassis.moveToPoint(-40, -60, 1200, {.forwards = true, .maxSpeed = 80});
    */
}
